...
export function getCrystalDirectory(): string {
  ...
  if (isInstalledApp()) {
    console.log('[foozol] Running as installed app, using ~/.crystal');
  ...
  if (process.env.__CFBundleIdentifier === 'com.stravu.crystal' && !app.isPackaged) {
    console.log('[foozol] Detected running inside foozol development, using ~/.crystal_dev for isolation');
