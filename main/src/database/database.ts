...
export class DatabaseService {
  ...
  private runMigrations(): void {
    ...
    const hasClosedPanelTypesColumn = sessionTableInfoFavorite.some((col: SqliteTableInfo) => col.name === 'closed_panel_types');

    if (!hasClosedPanelTypesColumn) {
      this.db.prepare("ALTER TABLE sessions ADD COLUMN closed_panel_types TEXT").run();
      console.log('[Database] Added closed_panel_types column to sessions table');
    }
  ...
  getClosedPanelTypes(sessionId: string): string[] {
    const row = this.db.prepare('SELECT closed_panel_types FROM sessions WHERE id = ?').get(sessionId) as { closed_panel_types: string | null } | undefined;
  ...
  addClosedPanelType(sessionId: string, panelType: string): void {
    ...
    if (!currentClosed.includes(panelType)) {
      ...
      this.db.prepare('UPDATE sessions SET closed_panel_types = ? WHERE id = ?').run(JSON.stringify(currentClosed), sessionId);
      console.log(`[Database] Added ${panelType} to closed panel types for session ${sessionId}`);
    }
  }
  ...
  removeClosedPanelType(sessionId: string, panelType: string): void {
    ...
    if (index > -1) {
      ...
      this.db.prepare('UPDATE sessions SET closed_panel_types = ? WHERE id = ?').run(JSON.stringify(currentClosed), sessionId);
      console.log(`[Database] Removed ${panelType} from closed panel types for session ${sessionId}`);
    }
  }
