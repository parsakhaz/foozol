import { MessageTransformer, UnifiedMessage, ToolCall, ToolResult, MessageSegment } from './MessageTransformer';
import { stripAnsi } from '../../../../utils/sanitizer';
...
export class CodexMessageTransformer implements MessageTransformer {
  ...
  /**
   * Clean text content by stripping ANSI escape sequences
   * These can appear in terminal output from Codex
   */
  ...
  transform(rawOutputs: CodexRawOutput[]): UnifiedMessage[] {
    const rawMessages: UnifiedMessage[] = [];
    ...
    // Merge consecutive messages with the same role and compatible segment types
    return this.mergeConsecutiveMessages(rawMessages);
  }

  /**
   * Merge consecutive messages with the same role to reduce visual clutter
   * Only merges messages that have compatible segment types (text, thinking)
   */
  private mergeConsecutiveMessages(messages: UnifiedMessage[]): UnifiedMessage[] {
    ...
    let currentGroup: UnifiedMessage | null = null;

    for (const message of messages) {
      // Check if we can merge this message with the current group
      const canMerge = currentGroup &&
        currentGroup.role === message.role &&
        this.canMergeSegments(currentGroup.segments, message.segments);

      if (canMerge && currentGroup) {
        // Merge segments into current group
        for (const segment of message.segments) {
          if (segment.type === 'text') {
            // Find existing text segment to append to, or add new one
            const lastSegment = currentGroup.segments[currentGroup.segments.length - 1];
            if (lastSegment && lastSegment.type === 'text') {
              // Append with single newline separator (minimal whitespace)
              lastSegment.content = lastSegment.content + '\n' + segment.content;
            } else {
              currentGroup.segments.push(segment);
            }
          } else if (segment.type === 'thinking') {
            // Find existing thinking segment to append to, or add new one
            const lastSegment = currentGroup.segments[currentGroup.segments.length - 1];
            ...
            // For other segment types, just add them
            currentGroup.segments.push(segment);
          }
        }
        ...
        // Can't merge - push current group (if any) and start new one
        if (currentGroup) {
          merged.push(currentGroup);
        }
        // Start a new group with a copy of the message
        currentGroup = {
          ...message,
  ...
  /**
   * Check if two sets of segments can be merged
   * Only allows merging pure text/thinking messages - not tool calls, errors, etc.
   */
  private canMergeSegments(existingSegments: MessageSegment[], newSegments: MessageSegment[]): boolean {
    // Only merge if all segments are text or thinking type
    const mergeableTypes = ['text', 'thinking'];

    for (const segment of existingSegments) {
      if (!mergeableTypes.includes(segment.type)) {
        return false;
      }
    }
  ...
  private parseOutput(output: CodexRawOutput): UnifiedMessage | null {
    ...
    if (output.type === 'stdout' || output.type === 'stderr') {
      ...
      if (parsedData && typeof parsedData === 'string' && parsedData.trim()) {
        const cleanedContent = this.cleanText(parsedData as string);
  ...
  private parseJsonMessage(message: unknown, timestamp?: string | Date): UnifiedMessage | null {
    ...
    if (typeof message === 'object' && message !== null && 'msg' in message) {
      ...
      if (msg.type === 'assistant_message' || msg.type === 'agent_message' || msg.type === 'text') {
        ...
        const rawContent = msg.message || msg.text || msg.content || '';
        const content = this.cleanText(rawContent);
        if (!content.trim()) return null;
    ...
    if (msg.type === 'assistant_response' || msg.type === 'assistant' || msg.type === 'text') {
      const rawContent = (typeof msg.content === 'string' ? msg.content : '') || (typeof msg.text === 'string' ? msg.text : '') || JSON.stringify(message);
      const content = this.cleanText(rawContent);
    ...
    if (msg.type === 'item.completed' && msg.item) {
      ...
      if (item.type === 'reasoning') {
        const rawContent = typeof item.text === 'string' ? item.text : '';
        const content = this.cleanText(rawContent);
        if (!content.trim()) return null;
